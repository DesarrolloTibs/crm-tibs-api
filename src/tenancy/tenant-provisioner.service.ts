    ...
    try {
      ...
      await queryRunner.query(`
        ...
        CREATE TABLE IF NOT EXISTS "${schemaName}".activities (
          ...
          "userId" uuid NULL REFERENCES "${schemaName}".users(id) ON DELETE SET NULL,
...
Redirección: Deriva con un ejecutivo especializado si hay inconformidades, quejas, molestia o si el cliente lo solicita.`;
      ...
      await queryRunner.query(
        `INSERT INTO "${schemaName}".ai_sub_agents (key, name, description, context, tools, temperature, "isActive") VALUES
          ...
          ('soporte_atencion', 'Sub-Agente de Soporte', 'Atiende incidencias de soporte, quejas, dudas técnicas, deriva con un ejecutivo especializado y genera tickets de soporte en la mesa de ayuda (Helpdesk).', $3, $7::jsonb, 0.50, true),
        ...
        [
          ...
          soportePrompt,
          generalPrompt,
          JSON.stringify(['registerContact', 'updateContact', 'createOpportunity', 'modifyOpportunity', 'consult_product_catalog']),
          JSON.stringify(['registerContact', 'updateContact', 'checkAvailability', 'createActivity']),
          JSON.stringify(['registerContact', 'updateContact', 'createTicket']),
        ]
      );
