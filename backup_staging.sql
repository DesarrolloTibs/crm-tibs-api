...
COPY staging.activities (id, date, activity, "opportunityId", "userId", "activityType", "clientId", flaghistory) FROM stdin;
...
COPY staging.clients (id, nombre, apellido, correo, empresa, puesto, telefono, estatus, ejecutivo_id, category) FROM stdin;
...
COPY staging.expenses (id, fecha, concepto, monto, client_id, opportunity_id, usuario_id, "receiptUrl", "createdAt") FROM stdin;
\.
...
COPY staging.interactions (id, comment, created_at, opportunity_id) FROM stdin;
...
COPY staging.nueva_tabla (id, date, activity, "activityType", "opportunityId", "userId") FROM stdin;
...
COPY staging.opportunities (id, nombre_proyecto, cliente_id, empresa, ejecutivo_id, etapa, monto_licenciamiento, monto_servicios, monto_total, moneda, linea_negocio, tipo_entrega, licenciamiento, proposal_document_path, archived, "tipoCambio", description, estimated_closure_date, "createdAt") FROM stdin;
...
COPY staging.opportunity_trackings (id, opportunity_id, stage, "changedAt", changed_by_id) FROM stdin;
...
COPY staging.reminders (id, title, date, opportunity_id) FROM stdin;
...
COPY staging.users (id, username, email, password, role, "isActive", "profileImageUrl", reset_password_token, reset_password_expires) FROM stdin;
